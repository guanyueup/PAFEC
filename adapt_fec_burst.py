...
def num_analysis(l):
    ...
    return result
def decision_maker(p_list,result_path,tag,lr,last_max_len):
    rr = 0.0
    p_ration = 1.0
    ...
    #     rr = redundary
    alph = 0.3
    max_loss_len = len(p_list)-1
    if last_max_len != 0:
        max_loss_len = int((1-alph)*max_loss_len + alph* last_max_len)

    rr = round((max_loss_len / 15)/1.5,2)
    p_ration = round((max_loss_len / 15)/3,2)

    with open(f'{result_path}/sim_result/{tag}/loss_pattern_{lr}.csv','a') as f:
        f.write(str(p_list)+'\n')
    return rr,p_ration,max_loss_len
    
    
def exc_loss(pack_all,loss_dic,packet_list,fec_num, fec_num1, front, reslut_path,tag,lr):
    pack_len = len(packet_list)
    total_sum = 0
    total_sum1 = 0
    pos_list = []
    #(i >= pack_len and i < pack_len + fec_num)):
    for i in range(pack_len+fec_num+fec_num1):
        if loss_dic[pack_all+i+1]==1:
            if (i < front  or (i >= pack_len + fec_num and i < pack_len+fec_num+fec_num1)):
                    total_sum+=1
            elif (i >= front  and i < pack_len + fec_num):
                    total_sum1+=1
    for i in range(pack_len+fec_num+fec_num1):
        if loss_dic[pack_all+i+1]==1:
            if i < front and total_sum > fec_num:
    ...
    with open(f'{reslut_path}/sim_result/{tag}/not_repair_num_{lr}.csv','a') as f:
...
def process_h264_file(input_path):
    ...
    if muti_lr > 2:
        ...
        red_rate = [0.3,0.35,0.4,0.45,0.5,0.6,0.65,0.7,0.75,0.8]
        red_rate = [muti_lr*rr for rr in red_rate]
    ...
    for lr,rr in zip(loss_rate,red_rate):
        lr = round(lr,3)
        rr = round(rr,3)
        loss_dic, len_list = network_trace_load(lr,tag,15)
        ...
        with open(f'{reslut_path}/lossrate_{lr}_redundancy_{rr}.h264','wb') as wf:
            ...
            for i,nalu in enumerate(nalus):
                ...
                if frame_type == 'IDR' or frame_type == 'I':
                    ...
                    exc_loss(current_packages,loss_dic,packet_list,fec_num,fec_num1,len(packet_list),reslut_path, tag,lr)
                    ...
                    if current_packages-loss_future > 1000: #and threshlod > 1.0:
                        p_list = num_analysis(len_list[current_packages-1000:current_packages-15])
                        rr,p_ratio,loss_all = decision_maker(p_list,reslut_path,tag,lr,loss_all)
                        loss_future = current_packages
                elif frame_type == 'P':
                    ssim = float(ssim_value[index])
                    index += 1
                    fec_num1 = 0
                    ...
                    if threshlod > 1.0:
                        ...
                        fec_num = math.ceil(muti_f* rr * len(packet_list))
                        if muti_f <1.0:
                            front = len(packet_list)*p_ratio
                        #fec_num2 = int(0.5* rr * (len(packet_list)-front))
                    exc_loss(current_packages,loss_dic,packet_list,fec_num,fec_num1,front,reslut_path, tag,lr)
                    current_packages += (len(packet_list)+fec_num+fec_num1)
