                ...
                if loss_future == 0:
                    if random.random() < lr:
                        ...
                        if loss_all <= current_packages * lr:
                            loss_future -= 1
                            loss_all += 1
                            f.write(str(current_packages)+','+str(1)+','+str((loss_all/current_packages*100))+'%\n')
                        else:
                            loss_future = 0
                            f.write(str(current_packages)+','+str(0)+','+str((loss_all/current_packages*100))+'%\n')
                    else:
                        f.write(str(current_packages)+','+str(0)+','+str((loss_all/current_packages*100))+'%\n')
                else:
                    loss_future -= 1
        ...
        with open(f'{reslut_path}/network_trace/{tag}/{lr}_loss_record.csv','w') as f:
            for i,st in enumerate(status):
                if st:
                    ...
                    str_st = '0'
                f.write(str(i+1)+','+str_st+','+str((loss/(i+1)*100))+'%\n')
