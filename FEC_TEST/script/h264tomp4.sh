...
# 输入文件夹
input_dir="/home/zzp/FEC_Test/720P_CAFEC"
...
do
    ...
    # 执行封装命令，自动生成PTS，防止警告
    #ffmpeg -fflags +genpts -r 25 -i -c copy 
    #ffmpeg -fflags +genpts -err_detect ignore_err -r 25 -i "$file"  -c:v copy -bsf:v h264_mp4toannexb "$output_file"
    ffmpeg -fflags +genpts -err_detect ignore_err -ec 2 -r 25 -i "$file" -c:v libx264 -preset ultrafast "$output_file" -y
