...
if __name__ == "__main__":
    ...
    path = video_path[0]
    if 'CAFEC' in path:
        file_name = f"{path}/sim_result/{tag}/ssim_psnr_CAFEC.csv"
    else:
        file_name = f"{path}/sim_result/{tag}/ssim_psnr_FEC.csv"
        ...
        with open(file_name, 'a') as f:
            ...
            for lr,rr in zip(loss_rate,red_rate):
                ...
                if not (os.path.exists(video1) and os.path.exists(video2)):
                    ...
                    ssim_vals, psnr_vals = calculate_metrics_multithread(video1, video2, max_workers=18)
                    f.write(f'{lr},{ssim_vals},{psnr_vals}\n')
